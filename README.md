...
## **Overview**
miFRED is a user-friendly pipeline to calculate functional redundancy (FRED) using as functional units 86 phenotypes predicted by MICROPHERRET machine learning models from KEGG orthologs (KO). 

The pipeline processes metagenomic samples by taking as input:
- shotgun metagenomic reads from each sample
...
The additional community-level metrics are also calculated:
- **FREDc_Tian**: community-redundancy as defined by Tian *et al.*
- **GSI**: Gini-Simpson Index of Alpha diversity
- **Rao**: Rao's quadratic entropy as a measure of functional dissimilarity

Outputs include several files storing the calculated FRED metrics, summary statistics and distribution plots. Intermediate results are also provided, including MICROPHERRET functional predictions, Jaccard distances between organisms and relative abundance values. To improve transparency and allow the users to check how much community is consider to measure FRED, mapping rates and changes in the community size at each steps are reported.
